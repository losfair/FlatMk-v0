...
impl CPtr {
    pub unsafe fn call(&self, p0: i64, p1: i64, p2: i64, p3: i64) -> i64 {
        let syscall: unsafe extern "C" fn (i64, i64, i64, i64, i64, i64, i64) -> i64 = core::mem::transmute(_do_syscall as usize);
        syscall(self.0 as _, p0, p1, p2, p3, 0, SyscallIndex::Call as u32 as _)
    }
...
impl RootCap {
    ...
    pub unsafe fn make_any_x86_port(&self, del: &'static mut Delegation) -> Option<AnyX86Port> {
        if let Some(cptr) = alloc_cap() {
            if self.0.call(cptr.0 as _, RootTaskCapRequest::AnyX86IoPort as u32 as _, del as *mut _ as i64, 0) == 0 {
                Some(AnyX86Port {
                    cap: cptr,
                })
            } else {
